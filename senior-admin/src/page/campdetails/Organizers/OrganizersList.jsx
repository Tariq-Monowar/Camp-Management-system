...
const OrganizersList = () => {
  ...
  useEffect(() => {
    const getAllData = async () => {
      try {
        ...
        const filteredData = data.filter((item) => item.isHedeleget === "false");
  ...
  const deleteDelegate = async (id, selectImage) => {
    ...
    const userConfirmed = window.confirm("আপনি এই সংগঠক কে মুছে ফেলতে চান?");
    if (userConfirmed) {
      try {
  ...
  const handlePrint = useReactToPrint({
    ...
    documentTitle: "অংশগ্রহণ কৃত সংগঠক তালকা",
      ...
      <div ref={componentRef} className="homePageContainer">
        <div style={{ marginBottom: "100px" }} className="regimentLest">
          <h1 className="regimentTitle">
            অংশগ্রহণ কৃত সংগঠক সংখ্যা: {allDeligatet?.length}
          </h1>
          <div className="downloadeShoit">
            <FaFilePdf className="pdfDownloade" onClick={handlePrint} />
            <DownloadTableExcel
              filename="অংশগ্রহণ কৃত সকল সংগঠক তালিকা"
              sheet="অংশগ্রহণ কৃত সকল সংগঠক তালিকা"
          ...
          <div className="regCreateBrnList">
            {/* <button onClick={() => navigate("/regimrntwisedOrganizeres")} style={{width: "230px"}} className="regimentBtn">
              রেজিমেন্ট ভিত্তিক তালিকা
            </button> */}
            <button
               onClick={() => navigate("/branchwisedOrganizeres")}

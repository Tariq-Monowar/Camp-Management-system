...
const RegimrntwisedOrganizeres = () => {
  ...
  const [regimentWisedList, setRegimentWisedList] = useState([]);
  ...
  const groupedObject = groupBy(allDelegate, "regiment");
        ...
        <div className="regwlist regimentLest">
          ...
          <div ref={tableRef}>
            {Object.keys(groupedObject).map((regiment, index) => {
              if (regiment && groupedObject[regiment].length > 0){ 
              return (
                <div key={index}>
                      ...
                      {groupedObject[regiment].map(
                        (delegate, delegateIndex) => (
                          <tr key={delegateIndex}>
                            ...
                            <td className="branceName">{delegate.brance}</td>
                            <td className="deligateCode">
                              44.24.
                              {String(delegate.delegatId).padStart(4, "0")}
                            </td>
                            <td>
                              <span style={{fontWeight: "normal"}} onClick={() => goToDetalseInfo(delegate.id)} className="detalseInfo">
                                বিস্তারিত তথ্য
                              </span>
                            </td>
                          </tr>
...
export default RegimrntwisedOrganizeres;

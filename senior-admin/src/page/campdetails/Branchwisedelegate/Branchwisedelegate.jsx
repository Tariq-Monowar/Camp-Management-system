...
const Branchwisedelegate = () => {
  ...
  const combinedArray = allBrance.map(obj1 => {
    const matchingObjects = allDeligate.filter(obj2 => obj2.brance === obj1.branceName);
    const numberOfPresent = matchingObjects.length;
    const chocos = matchingObjects.filter(data => data.organizationalvalues === "চৌকস").length
    const ogropothic = matchingObjects.filter(data => data.organizationalvalues === "অগ্রপথিক").length
    const dhiman = matchingObjects.filter(data => data.senOrganizationalvalues === "ধীমান").length
    const ovijathri = matchingObjects.filter(data => data.senOrganizationalvalues === "অভিযাত্রী").length
    const dhiman_ogrophotic = matchingObjects.filter(data => data.senOrganizationalvalues === "ধীমান" &&  data.organizationalvalues === "অগ্রপথিক").length
    const ovijatri_ogrophotic = matchingObjects.filter(data => data.senOrganizationalvalues === "অভিযাত্রী" &&  data.organizationalvalues === "অগ্রপথিক").length
    ...
    <>
      <div ref={componentRef} className="homePageContainer">
        <div className="regimentLest">
          <h1 className="regimentTitle">অংশগ্রহণকৃত শাখাঃ {combinedArray?.length}</h1>
          ...
          <table ref={tableRef} className="regimentTable">
            ...
            {allBrance &&
              ...
              combinedArray.map((percent, index) => {
                const { branceName, delegateDarjo, numberOfPresent, branceCode, chocos, ogropothic, dhiman, ovijathri, dhiman_ogrophotic, ovijatri_ogrophotic } = percent;
                let percentage = (Number(numberOfPresent) / Number(delegateDarjo)) * 100;
